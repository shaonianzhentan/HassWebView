...
namespace HassWebView.Core.Platforms.MacCatalyst;
...
#pragma warning restore CA1422
    ...
    private async void InjectBridgeProxyScript(WebView platformView)
    {
        ...
        var proxyScript = GenerateMacCatalystProxyScript();
    ...
    private string GenerateMacCatalystProxyScript()
    {
        ...
        {
            script.AppendLine($@"
    console.log('HassJsBridge: Creating MacCatalyst proxy for \'{bridgeName}\'.');
        ...
        get(target, propKey, receiver) {{
            return (...args) => {{
                ...
                if (window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.hassBridge) {{
                    ...
                    console.error(`HassJsBridge: MacCatalyst native bridge not found for '{bridgeName}'.`);
                }}
            }};
        }}
    }}); ");
